...
def subtitle(url,name):
	print(name)                                #finding extension for subtitle
	sp='http://ccsubs.com/video/yt:'
	sp=sp+url[33:]
	nm=sp+r'/'+name+'/download?format=srt&lang=en'
	down_sub(nm,name)


def make_choice(url,name_file):			                #make choice of whether audio or video	
	myvid= pafy.new(url)
	...
	if(int(i) is 1):
		...
		subtitle(url,name_file)																				 #subtitle download
		print('Downloaded')	
	else:
...
def find(url):                                           # Finding top 15 matches. Print their names and durations
	...
	if(i is 1):
		...
		j=1
		for ii in stt:
				href[j-1]="https://www.youtube.com/"+href[j-1]
				print(str(j)+". "+ii)
				j+=1;
